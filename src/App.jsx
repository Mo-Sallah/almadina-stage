...
const TRANSLATIONS = {
  ar: {
    ...
    noLoginRequired: "يتطلب حساب ولي أمر للمشاركة",
        showcaseTitle: "المدينة تبحث عن نجمها القادم",
        showcaseSub: "منصة خاصة بمسابقة المدينة ستيج والتي تنظمها جمعية البر بالمدينة المنورة وجمعية أطفال طيبة للأطفال في المدينة المنورة  من سن 6 إلى 12 عامًا.",
    ...
    cat1: "لبنات الاستيج (الأعمار 6–8)",
    cat2: "رواد الاستيج (الأعمار 9–12)",
    cat3: "رائدات الاستيج (الأعمار 9–12)",
  ...
  en: {
    ...
    cat1: "Lubnat Al-Istij (Ages 6–8)",
    cat2: "Ruwwad Al-Istij (Ages 9–12)",
    cat3: "Ra'idat Al-Istij (Ages 9–12)",
...
export default function App() {
  ...
  // New fields: club registration, nationality, Al-Bir beneficiary
  const [registeredInClub, setRegisteredInClub] = useState('');
  const [nationality, setNationality] = useState('');
  const [birBeneficiary, setBirBeneficiary] = useState('');
  ...
  const getAgeCategoryLabel = (age, gender) => {
    if (!age) return '';
    const ageNum = parseInt(age);
    if (ageNum >= 6 && ageNum <= 8) {
      return lang === 'ar' ? "لبنات الاستيج (الأعمار 6–8)" : "Lubnat Al-Istij (Ages 6–8)";
    } else if (ageNum >= 9 && ageNum <= 12) {
      if (gender === 'male') {
        return lang === 'ar' ? "رواد الاستيج (الأعمار 9–12)" : "Ruwwad Al-Istij (Ages 9–12)";
      } else if (gender === 'female') {
        return lang === 'ar' ? "رائدات الاستيج (الأعمار 9–12)" : "Ra'idat Al-Istij (Ages 9–12)";
      }
      return lang === 'ar' ? "رواد/رائدات الاستيج (الأعمار 9–12)" : "Ruwwad/Ra'idat Al-Istij (Ages 9–12)";
    }
    return '';
  };
  ...
  const loadSubmissionsData = async () => {
    try {
      ...
      if (!subResponse.ok) throw new Error('Server error');
  ...
  const handleFormSubmission = async (e) => {
    ...
    if (!fullName || !nationalId || !childAge || !parentAgreed || !nationality || !registeredInClub || !birBeneficiary) {
    ...
    const calculatedCategory = getAgeCategoryLabel(childAge, childGender);
    ...
    formData.append('registeredInClub', registeredInClub);
    formData.append('birBeneficiary', birBeneficiary);
    ...
    const uploadWithProgress = () => new Promise((resolve, reject) => {
      ...
      // Phase 1: 5% → 75% while bytes travel to the server
      xhr.upload.onprogress = (event) => {
        if (event.lengthComputable) {
          const percent = 5 + Math.round((event.loaded / event.total) * 70);
      ...
      // Phase 2: 75% → 95% ticking while server processes Cloudinary
      xhr.upload.onloadend = () => {
        if (settled) return;
        let progress = 75;
        setUploadProgress(progress);
        setProgressStateText(lang === 'ar' ? 'جاري رفع الفيديو على المنصة السحابية...' : 'Processing video on cloud...');
        cloudInterval = setInterval(() => {
          if (settled) { clearInterval(cloudInterval); return; }
          if (progress < 94) {
            progress += 1;
            setUploadProgress(progress);
          }
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          try {
            ...
            settle(reject, new Error('Invalid server response'));
          }
        } else {
      ...
      xhr.onerror   = () => settle(reject, new Error(lang === 'ar' ? 'خطأ في الشبكة — تحقق من الاتصال' : 'Network error — check your connection'));
      xhr.ontimeout = () => settle(reject, new Error(lang === 'ar' ? 'انتهت مهلة الرفع' : 'Upload timed out after 5 minutes'));
    ...
    try {
      ...
      showToast(
        err.message && err.message.length < 200
          ? err.message
          : (lang === 'ar' ? 'تعذر الاتصال بالخادم. يرجى المحاولة مرة أخرى.' : 'Upload failed. Please try again.'),
        "error"
      );
    } finally {
      ...
      if (parentMode === 'register') {
        ...
        try {
          const response = await fetch(`${apiUrl}/api/user/register`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: emailInput.trim(), password: passwordInput, fullName: regFullName.trim(), mobile: regMobile.trim(), age: 25, nationalId: 'N/A' }),
          });
          const result = await response.json();
          if (result.status === 'success') {
        ...
        {currentView === 'submission' && (
                ...
                <div className="relative z-10 flex flex-col lg:flex-row h-full">
                  ...
                  <div className="flex-1 p-8 sm:p-10 flex flex-col justify-between gap-6">
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                      <button
                        onClick={() => {
                          if (!loggedInUser) { setShowAuthModal(true); }
                          else { document.getElementById('registration-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
                        }}
                  ...
                  {/* Right: photo */}
                  <div className="lg:w-[42%] relative overflow-hidden rounded-b-[2rem] lg:rounded-b-none lg:rounded-s-none lg:rounded-e-[2rem]" style={{minHeight: '360px'}}>
                    <img
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-10">
            ...
            {/* Registration Form Field */}
                          <div className={`lg:col-span-8 bg-white rounded-[2.5rem] border-4 ${!loggedInUser ? 'border-[#90CFB0]' : 'border-[#F5876C]'} shadow-xl p-6 sm:p-10 relative `}>
              ...
              {!loggedInUser ? (
                <div className="py-6 px-2 space-y-5">
                  ...
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-base font-bold text-slate-700 tracking-tight">
                      {lang === 'ar' ? 'إحصائيات المسابقة' : 'Competition Statistics'}
                    </h3>
                    <span className="text-[11px] text-slate-400 font-semibold bg-slate-100 px-3 py-1 rounded-full">
                      {lang === 'ar' ? 'لحظي' : 'Live'}
                      <span className="inline-block w-1.5 h-1.5 rounded-full bg-green-400 ml-1 mb-0.5 animate-pulse"></span>
                    </span>
                  </div>

                  {/* Chart — Brand Palette Donut + Cards */}
                  {(() => {
                    const totalVotes = submissions.reduce((acc, s) => acc + (s.votes || 0), 0);
                    // Brand colors from palette: purple, teal, green, coral/salmon
                    const stats = [
                      {
                        label: lang === 'ar' ? 'مشاركون' : 'Participants',
                        sublabel: lang === 'ar' ? 'مسجّلون' : 'Registered',
                        value: submissions.length || 0,
                        color: '#6A5E9E',   // brand purple
                        bg: '#EDEAF8',
                        accent: '#9B8ED4',
                      },
                      {
                        label: lang === 'ar' ? 'فئات' : 'Categories',
                        sublabel: lang === 'ar' ? 'عمرية' : 'Age groups',
                        value: 3,
                        color: '#60A7BD',   // brand teal
                        bg: '#DFF1F7',
                        accent: '#8FCCDC',
                      },
                      {
                        label: lang === 'ar' ? 'أصوات' : 'Votes',
                        sublabel: lang === 'ar' ? 'جمهور' : 'Public',
                        value: totalVotes,
                        color: '#6AB28D',   // brand green
                        bg: '#DFF2EB',
                        accent: '#90CFB0',
                      },
                      {
                        label: lang === 'ar' ? 'أولياء' : 'Parents',
                        sublabel: lang === 'ar' ? 'حسابات' : 'Accounts',
                        value: usersCount,
                        color: '#E37C8D',   // brand coral-pink
                        bg: '#FCEEF1',
                        accent: '#EDA8B4',
                      },
                    ];
                    const total = stats.reduce((a, s) => a + (s.value || 0), 0) || 1;
                    ...
                    return (
                      <div className="flex flex-col gap-5 px-1 pt-2 pb-3">
                        ...
                        <div className="flex items-center gap-6">
                          <div className="relative flex-shrink-0" style={{width: 140, height: 140}}>
                            <svg viewBox="0 0 100 100" width="140" height="140" style={{transform: 'rotate(-90deg)'}}>
                              {/* Background ring */}
                              <circle cx={cx} cy={cy} r={R} fill="none" stroke="#F1F0F8" strokeWidth="13" />
                              {segments.map((seg, i) => (
                                <circle
                                  key={i}
                                  cx={cx} cy={cy} r={R}
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                              <span className="text-[28px] font-black text-slate-800 leading-none">{submissions.length}</span>
                              <span className="text-[11px] font-bold text-slate-400 mt-1">{lang === 'ar' ? 'مشارك' : 'total'}</span>
                            </div>
                          </div>
                            ...
                            {segments.map((seg, i) => {
                              ...
                              return (
                                <div key={i} className="flex items-center gap-2.5">
                                  ...
                                  <span className="text-[13px] font-bold text-slate-600 flex-1">{seg.label}</span>
                                  <div className="flex items-center gap-2">
                                    <div className="w-20 h-2 rounded-full overflow-hidden" style={{background: seg.bg}}>
                                      <div className="h-full rounded-full transition-all duration-700" style={{width: `${Math.max(4, pct)}%`, background: seg.color}} />
                                    </div>
                                    <span className="text-[12px] font-extrabold w-8 text-right" style={{color: seg.color}}>{seg.value}</span>
                                  </div>
                                </div>
                              );
                            })}
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          {stats.map((s, i) => (
                            <div
                              key={i}
                              className="rounded-2xl flex flex-col items-center justify-center py-3.5 px-2 gap-1"
                              style={{background: s.bg}}
                            >
                              <span className="text-[22px] font-black leading-none" style={{color: s.color}}>{s.value}</span>
                              <span className="text-[11px] font-bold text-center leading-tight" style={{color: s.accent}}>{s.label}</span>
                              <span className="text-[10px] font-medium text-slate-400 text-center leading-tight">{s.sublabel}</span>
                            </div>
                          ))}
                        </div>
                ...
                <form id="registration-form" onSubmit={handleFormSubmission} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          value={childAge}
                          onChange={(e) => { setChildAge(e.target.value); setChildGender(''); }}
                          required
                          className={`w-full bg-slate-50 border-2 border-slate-200 rounded-2xl font-semibold focus:outline-none focus:ring-4 focus:ring-[#6A5E9E]/10 focus:border-[#8a7eb8] transition-all text-sm appearance-none py-4 ${childAge ? 'text-slate-900' : 'text-slate-400'} ${lang === 'ar' ? 'pl-10 pr-5' : 'pr-10 pl-5'}`}
                        >
                          <option value="" disabled hidden>{lang === 'ar' ? 'اختر العمر' : 'Select age'}</option>
                  ...
                  {/* Gender Dropdown — only shown for ages 9–12 */}
                  {childAge && parseInt(childAge) >= 9 && (
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">
                        {lang === 'ar' ? 'جنس الطفل' : 'Child Gender'} <span className="text-rose-500">*</span>
                      </label>
                      <div className="relative">
                        <select
                          value={childGender}
                          ...
                          required
                          className={`w-full bg-slate-50 border-2 border-slate-200 rounded-2xl font-semibold focus:outline-none focus:ring-4 focus:ring-[#6A5E9E]/10 focus:border-[#8a7eb8] transition-all text-sm appearance-none py-4 ${childGender ? 'text-slate-900' : 'text-slate-400'} ${lang === 'ar' ? 'pl-10 pr-5' : 'pr-10 pl-5'}`}
                        >
                          <option value="" disabled hidden>{lang === 'ar' ? 'اختر الجنس' : 'Select gender'}</option>
                  ...
                  {(parseInt(childAge) <= 8 || childGender) && (
                    <div className="p-4 bg-[#E3F2EB] border-2 border-[#6AB28D]/50 rounded-2xl text-xs sm:text-sm text-[#3a8060] font-bold flex items-center gap-2">
                      ...
                      <span>{t.ageCategory}: {getAgeCategoryLabel(childAge, childGender)}</span>
                    </div>
                  )}
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">
                      {lang === 'ar' ? 'الجنسية' : 'Nationality'} <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={nationality}
                      ...
                      required
                      placeholder={lang === 'ar' ? 'مثال: سعودي، مصري...' : 'e.g. Saudi, Egyptian...'}
                      className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl text-slate-900 font-semibold placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-[#6A5E9E]/10 focus:border-[#8a7eb8] transition-all text-sm"
                    />
                  </div>

                  {/* Registered in Club & Al-Bir Beneficiary Row */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">

                    {/* Registered in Club */}
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">
                        {lang === 'ar' ? 'مسجل في النادي' : 'Registered in Club'} <span className="text-rose-500">*</span>
                      </label>
                      <div className="flex gap-3">
                        {['yes', 'no'].map((val) => (
                          <button
                            key={val}
                            type="button"
                            onClick={() => setRegisteredInClub(val)}
                            className={`flex-1 py-4 rounded-2xl font-bold text-sm border-2 transition-all ${
                              registeredInClub === val
                                ? 'bg-[#6A5E9E] border-[#4a4080] text-white shadow-md'
                                : 'bg-slate-50 border-slate-200 text-slate-500 hover:border-[#8a7eb8] hover:text-[#6A5E9E]'
                            }`}
                          >
                            {val === 'yes' ? (lang === 'ar' ? 'نعم' : 'Yes') : (lang === 'ar' ? 'لا' : 'No')}
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">
                        {lang === 'ar' ? 'مستفيد من جمعية البر' : 'Al-Bir Association Beneficiary'} <span className="text-rose-500">*</span>
                      </label>
                      <div className="flex gap-3">
                        {['yes', 'no'].map((val) => (
                          <button
                            key={val}
                            type="button"
                            onClick={() => setBirBeneficiary(val)}
                            className={`flex-1 py-4 rounded-2xl font-bold text-sm border-2 transition-all ${
                              birBeneficiary === val
                                ? 'bg-[#AC6E97] border-[#8a5078] text-white shadow-md'
                                : 'bg-slate-50 border-slate-200 text-slate-500 hover:border-[#AC6E97] hover:text-[#AC6E97]'
                            }`}
                          >
                            {val === 'yes' ? (lang === 'ar' ? 'نعم' : 'Yes') : (lang === 'ar' ? 'لا' : 'No')}
                          </button>
                  ...
                  {/* Submission Limit Disclaimer */}
                  {(() => {
                    const MAX_SUBMISSIONS = 5;
                    const used = userSubmissions.length;
                    const remaining = MAX_SUBMISSIONS - used;
                    const limitReached = used >= MAX_SUBMISSIONS;

                    return limitReached ? (
                      <div className="bg-[#FBE8EB] border-2 border-[#E37C8D]/50 rounded-3xl p-6 flex flex-col items-center gap-4 text-center">
                        <div className="w-14 h-14 bg-[#E37C8D]/15 rounded-2xl flex items-center justify-center">
                          <XCircle className="w-7 h-7 text-[#E37C8D]" />
                        </div>
                        <div className="space-y-1">
                          <p className="font-extrabold text-[#2d2650] text-sm">
                            {lang === 'ar' ? 'وصلت للحد الأقصى للتسجيل' : 'Registration Limit Reached'}
                          </p>
                          <p className="text-xs text-slate-500 font-semibold leading-relaxed">
                            {lang === 'ar'
                              ? `لقد سجّلت ${used} أطفال بالفعل. الحد الأقصى المسموح به لكل ولي أمر هو ${MAX_SUBMISSIONS} أطفال فقط.`
                              : `You have already registered ${used} children. The maximum allowed per parent is ${MAX_SUBMISSIONS}.`}
                          </p>
                        </div>
                      </div>
                    ) : (
                      <div className={`rounded-3xl p-5 border-2 flex items-start gap-3 ${
                        remaining === 1
                          ? 'bg-orange-50 border-orange-200'
                          : 'bg-[#EAE8F5] border-[#6A5E9E]/25'
                      }`}>
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
                          remaining === 1 ? 'bg-orange-100' : 'bg-[#6A5E9E]/10'
                        }`}>
                          <Users className={`w-5 h-5 ${remaining === 1 ? 'text-orange-500' : 'text-[#6A5E9E]'}`} />
                        </div>
                        <div className="space-y-0.5">
                          <p className={`font-bold text-sm ${remaining === 1 ? 'text-orange-700' : 'text-[#2d2650]'}`}>
                            {lang === 'ar'
                              ? `متبقي لك ${remaining} من ${MAX_SUBMISSIONS} تسجيلات`
                              : `${remaining} of ${MAX_SUBMISSIONS} registrations remaining`}
                          </p>
                          <p className="text-xs text-slate-500 font-semibold leading-relaxed">
                            {lang === 'ar'
                              ? `يمكن لولي الأمر تسجيل حتى ${MAX_SUBMISSIONS} أطفال. كل طفل يُسجَّل بطلب منفصل بفيديو خاص به.`
                              : `A parent may register up to ${MAX_SUBMISSIONS} children. Each child is submitted separately with their own video.`}
                          </p>
                        </div>
                      </div>
                    );
                    ...
                    >
                      ...
                      {!selectedPhotoFile ? (
                        ...
                        <div className="flex items-center justify-between p-3 bg-white border-2 border-teal-100 rounded-2xl shadow-sm gap-2">
                          ...
                          <button type="button" onClick={(e) => { e.stopPropagation(); setSelectedPhotoFile(null); if (photoFileInputRef.current) photoFileInputRef.current.value = ""; }} className="w-8 h-8 rounded-lg hover:bg-[#FEF0F2] text-slate-400 hover:text-rose-600 flex items-center justify-center transition-all">
                  ...
                  {/* Submit Action Button — hidden when limit reached */}
              ...
              <form onSubmit={handleUnifiedAuth} className={`space-y-3 ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
                ...
                {/* Step indicator for register */}
                              {authTab === 'parent' && parentMode === 'register' && (
                                  <div className="flex items-center gap-2 mb-1" style={{ flexFlow: 'row-reverse' }}>
                ...
                {authTab === 'parent' && parentMode === 'register' && registerStep === 2 && (
                  <>
                    <div>
                      <label className={`block text-xs font-bold text-slate-600 mb-1.5 ${lang === 'ar' ? 'text-right' : ''}`}>
                                              {lang === 'ar' ? 'اسم ولي الامر' : 'Full Name'}
                      </label>
                      <input
                        type="text"
                        required
